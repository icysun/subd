...
import os
import glob
from lib.common import load_dns_servers, load_next_sub, print_msg, user_abort


class SubNameBrute:
    def __init__(self, target, threads, process, process_num, dns_servers, next_subs,
    ...
    def _load_sub_names(self):
        _file = 'subDomainsBrute/dict/subnames_full.txt'
        normal_lines = []
        wildcard_lines = []
        wildcard_list = []
        regex_list = []
        ...
        for item in normal_lines[self.process_num::self.process]:
            self.priority += 1
            self.queue.put((self.priority, item))

        for item in wildcard_lines[self.process_num::self.process]:
    ...
    def _scan(self, j):
        ...
        while not self.queue.empty():
            try:
                item = self.queue.get(timeout=5.0)[1]
            ...
            try:
                ...
                if answers:
                    ...
                    self.outfile.flush()
                    try:
                        self.resolvers[j].query('11li123321.' + cur_sub_domain)
...
def run_process(target, threads, process, process_num, dns_servers, next_subs, scan_count, found_count, queue_size_list,
                tmp_dir):
    signal.signal(signal.SIGINT, user_abort)
    s = SubNameBrute(target=target, threads=threads, process=process, process_num=process_num,
                     dns_servers=dns_servers, next_subs=next_subs,
                     scan_count=scan_count, found_count=found_count, queue_size_list=queue_size_list,
                     tmp_dir=tmp_dir)
    s.run()

def main(target, threads, process, result):
    start_time = time.time()
    # make tmp dirs
    tmp_dir = 'tmp/%s_%s' % (target, int(time.time()))
    ...
    queue_size_list = multiprocessing.Array('i', process)

    try:
        print '[+] Init %s scan process.' % process
        for process_num in range(process):
            p = multiprocessing.Process(target=run_process,
                                        args=(target, threads, process, process_num,
